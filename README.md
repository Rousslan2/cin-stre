# 🎬 CinéStream - Plateforme de Streaming de Films

## Description

CinéStream est une plateforme moderne de streaming de films avec un design élégant, des effets 3D captivants, et un système d'abonnement complet. Ce projet comprend un front-end responsive et un back-end Node.js avec base de données SQLite.

## ✨ Fonctionnalités

### Front-End
- 🎨 Design moderne avec effets 3D et animations
- 📱 Interface responsive (mobile, tablette, desktop)
- 🎬 Lecteur vidéo personnalisé avec contrôles avancés
- 🔍 Système de recherche et filtrage de films
- 💳 Pages d'abonnement attractives
- 🔐 Système d'authentification (inscription/connexion)

### Back-End
- 🚀 Serveur Node.js avec Express
- 💾 Base de données SQLite
- 🔒 Authentification sécurisée avec bcrypt
- 📊 API RESTful pour gérer les films et utilisateurs
- 🎫 Système de gestion des abonnements

## 🛠️ Technologies Utilisées

- **Front-End**: HTML5, CSS3, JavaScript (Vanilla)
- **Back-End**: Node.js, Express.js
- **Base de données**: SQLite3
- **Sécurité**: bcryptjs, express-session
- **Icons**: Font Awesome 6

## 📦 Installation

1. **Installer Node.js** (si ce n'est pas déjà fait)
   - Téléchargez depuis: https://nodejs.org/

2. **Installer les dépendances**
   ```bash
   cd streaming-site
   npm install
   ```

3. **Démarrer le serveur**
   ```bash
   npm start
   ```

4. **Ouvrir dans le navigateur**
   ```
   http://localhost:3000
   ```

## 📁 Structure du Projet

```
streaming-site/
│
├── public/                 # Fichiers front-end
│   ├── css/               # Fichiers CSS
│   │   ├── style.css      # Styles principaux
│   │   ├── auth.css       # Styles authentification
│   │   ├── movies.css     # Styles catalogue
│   │   ├── subscription.css # Styles abonnements
│   │   └── player.css     # Styles lecteur
│   │
│   ├── js/                # Fichiers JavaScript
│   │   ├── main.js        # Script principal
│   │   ├── auth.js        # Gestion authentification
│   │   ├── movies.js      # Gestion catalogue
│   │   ├── subscription.js # Gestion abonnements
│   │   └── player.js      # Lecteur vidéo
│   │
│   ├── index.html         # Page d'accueil
│   ├── login.html         # Page de connexion
│   ├── register.html      # Page d'inscription
│   ├── movies.html        # Catalogue de films
│   ├── subscription.html  # Page d'abonnements
│   └── player.html        # Lecteur vidéo
│
├── server.js              # Serveur Node.js/Express
├── package.json           # Dépendances du projet
└── streaming.db           # Base de données (créée automatiquement)
```

## 🎯 Pages Principales

### 1. Page d'Accueil (index.html)
- Hero section avec effets 3D
- Cartes de films flottantes animées
- Présentation des fonctionnalités
- Plans d'abonnement
- Footer complet

### 2. Authentification
- **Inscription** (register.html): Création de compte
- **Connexion** (login.html): Authentification utilisateur

### 3. Catalogue (movies.html)
- Recherche de films
- Filtrage par genre
- Vue grille/liste
- Animations au scroll

### 4. Lecteur Vidéo (player.html)
- Lecteur personnalisé avec contrôles
- Informations détaillées du film
- Films similaires
- Raccourcis clavier

### 5. Abonnements (subscription.html)
- 3 plans (Basique, Standard, Premium)
- Tarification mensuelle/annuelle
- FAQ interactive
- Tableau comparatif

## 🔐 API Endpoints

### Authentification
- `POST /api/register` - Inscription
- `POST /api/login` - Connexion
- `POST /api/logout` - Déconnexion
- `GET /api/check-auth` - Vérifier l'authentification

### Films
- `GET /api/movies` - Liste de tous les films
- `GET /api/movies/:id` - Détails d'un film

### Abonnements
- `POST /api/subscribe` - Souscrire à un plan

## 🎨 Design Features

### Effets 3D
- Cartes de films avec effet de profondeur
- Animations de survol
- Transitions fluides
- Parallaxe sur certains éléments

### Animations
- Fade in au scroll
- Animations de flottement
- Transitions de page
- Micro-interactions

### Responsive
- Breakpoints optimisés
- Navigation mobile
- Grilles adaptatives
- Touch-friendly

## 🚀 Fonctionnalités Avancées

### Lecteur Vidéo
- Play/Pause (Espace ou K)
- Avance rapide/retour (Flèches)
- Volume (↑↓ ou M pour mute)
- Plein écran (F)
- Barre de progression interactive
- Contrôles qui se cachent automatiquement

### Système d'Abonnement
- 3 niveaux d'abonnement
- Tarification flexible
- Essai gratuit de 30 jours
- Comparaison des plans

## 💡 Pour le Développement

### Ajouter des Films
Les films sont stockés dans la base de données SQLite. Pour ajouter de nouveaux films, modifiez la section d'initialisation dans `server.js`.

### Personnaliser les Styles
Les variables CSS principales sont dans `:root` de `style.css`:
```css
--primary: #e50914;
--secondary: #f40612;
--dark: #0a0a0a;
```

### Ajouter des Fonctionnalités
Le code est modulaire. Chaque page a son propre fichier JS dans `/public/js/`.

## ⚠️ Notes Importantes

1. **Vidéos**: Ce projet utilise des URLs d'exemple. Pour un usage réel, vous devez:
   - Héberger vos vidéos sur un CDN
   - Mettre à jour les URLs dans la base de données
   - Considérer l'utilisation d'un service comme AWS S3, Cloudflare Stream, etc.

2. **Sécurité**: Pour la production:
   - Utilisez HTTPS
   - Ajoutez un système de tokens JWT
   - Implémentez des limites de taux (rate limiting)
   - Validez toutes les entrées utilisateur
   - Utilisez des variables d'environnement pour les secrets

3. **Performance**:
   - Optimisez les images
   - Implémentez du caching
   - Minifiez le CSS/JS
   - Utilisez un CDN

## 📝 Compte de Test

Après le démarrage du serveur, vous pouvez créer un compte ou utiliser les fonctionnalités sans authentification pour tester.

## 🎓 Pour Aller Plus Loin

### Améliorations Possibles
- Intégration de vraies APIs de films (TMDB, OMDB)
- Système de recommandations
- Chat en direct
- Watchlist personnalisée
- Historique de visionnage
- Système de notation et commentaires
- Mode sombre/clair
- Multi-langues
- Notifications push

### Intégrations
- Stripe pour les paiements réels
- AWS pour l'hébergement de vidéos
- Redis pour le caching
- MongoDB pour une base de données plus robuste
- Socket.io pour les fonctionnalités temps réel

## 🤝 Support

Pour toute question ou problème:
1. Vérifiez que Node.js est installé
2. Assurez-vous que le port 3000 est disponible
3. Vérifiez les logs du serveur dans la console

## 📄 Licence

Ce projet est un exemple éducatif. Libre d'utilisation et de modification.

## 🎉 Profitez de CinéStream !

Votre plateforme de streaming moderne est prête ! N'oubliez pas de personnaliser le contenu et d'ajouter vos propres films.

---

**Développé avec ❤️ pour offrir la meilleure expérience de streaming**
"# cin-stre" 
